...
function getRootParamsFromLayouts(layouts: Record<string, string[]>) {
  ...
  for (const [layoutPath, params] of sortedLayouts) {
    if (!rootLayout) {
      // First iteration, assume it's a root layout
      rootLayout = layoutPath
      rootParams = new Set<string>(params)
    } else if (isSubpath(rootLayout, layoutPath)) {
...
function isSubpath(parentLayoutPath: string, potentialChildLayoutPath: string) {
  ...
  const parentSegments = parentLayoutPath
    .split('/')
    .slice(0, -1)
    .filter(Boolean)
  const childSegments = potentialChildLayoutPath
    .split('/')
    .slice(0, -1)
    .filter(Boolean)
